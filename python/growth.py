...
def plot_afd():
    ...
    afd_g = (afd[-1] / afd[0]) ** alpha - 1
    alum_g = (alum[-1] / alum[0]) ** alpha - 1
    prof_g = (prof[-1] / prof[0]) ** alpha - 1
    gdp_g = (gdp[-1] / gdp[0]) ** alpha - 1
    wage_g = (wage[-1] / wage[0]) ** alpha - 1

    ax.plot(year, afd / 1e+6, '-', color=(.4,.4,.4), 
                lw=3, label=f'AFD ({afd_g:+.1%})')
    m = afd[-1] / 1e+6
    ax.set_ylabel('AFD [× 10⁹ CLP]')
    ax.set_ylim(0, 1.1 * m)

    ax2 = ax.twinx()
    ax2.plot(year, 100 * alum, 'k-', label=f'undergraduates ({alum_g:+.1%})')
    ax2.plot(year, 100 * prof, 'k--', label=f'professors ({prof_g:+.1%})')
    ax2.plot(year, 100 * gdp,  'k-.', label=f'GDP/capita ({gdp_g:+.1%})')
    ax2.plot(year, 100 * wage, 'k:', label=f'real wages ({wage_g:+.1%})')
    ax2.set_ylim(0, 110)
